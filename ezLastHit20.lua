...
function LastHitOnTick()
	...
	if ( Menu.AutoTurnOn and not (Menu.LastHitSettings.StopComboKey and Menu.ComboKey) ) 
		...
		--[[
		if Menu.Orbwalk.HarassChampions and GetTick() > me.nextBasicTime then
		local target = GetPriorityTarget()
...
function OnSendPacket(p)	
	...
	if Menu.BlockMovement and GetTick() < me.lastWindup
...
function UpdateSpellParticles()
	for sourceName, attackDetails in pairs(incomingDetails) do
		GetMissileParticle(attackDetails)
	end
end
...
function GetMyTimeToHit(target)
	...
	if myHero.range < 400 then
		return GetTick() + me.windupTime() - 50 + latencyDelay - Menu.LastHitSettings.LastHitDelay
	else
		return GetTick() + GetDistance(target.visionPos)/me.projSpeed + me.windupTime() + latencyDelay - 50 - Menu.LastHitSettings.LastHitDelay
	end
end
...
function GetTimeWhenHit(attackDetails, target)
	...
	if attackDetails.speed == 0 and attackDetails.spellParticle ~= nil then
		return attackDetails.startTime + attackDetails.delayOffset - latencyDelay
...
function refreshAttack(spellName)
    return (
		...
		or spellName == "Meditate"
		or spellName == "FioraFlurry"
    )
end
...
function OnAnimation(unit, animation)    
	...
	if incomingDetails[unit.name] then
		if GetTick() < incomingDetails[unit.name].windupTime and not animation:find("Attack") then
...
function OnLoad()	
		...
		Menu.LastHitSettings:addParam("LatencyDelay", "Calculate Ping time", SCRIPT_PARAM_ONOFF, false)
		...
		--Menu.Orbwalk:addParam("OrbwalkChampions", "Attack Champions on ComboKey", SCRIPT_PARAM_ONOFF, false)
