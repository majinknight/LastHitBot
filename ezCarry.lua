		...
		if (spell.name:find("Attack") or isSpellAttack(spell.name))
		 and GetTick() < me.lastCommandTime then
			...
			if me.baseAttackSpeed == nil then
				me.baseAttackSpeed = (1/spell.animationTime) / myHero.attackSpeed
...
function OnTick()
	...
	if (GetTick() > me.lastWindup or skillShot.noAnimation)  and ((skillShot.params.UseSpell and ezSkillShot.KeySettings.CastSpell)
		...
		if ValidTarget(enemy.enemyObj,trueRange) 
	...
	if ezSkillShot[myHero.charName .. "Settings"].Orbwalk and ezSkillShot.KeySettings.CastSpell and
		...
		--local currentTarget = GetHeroByName(ezSkillShot[myHero.charName .. "Settings"].currentTarget.name)
		
		--move to where he is going to be
		--[[if currentTarget and currentTarget.waypointDir then
			local predictDelayPos = currentTarget.lastPosVec + currentTarget.waypointDir * currentTarget.moveSpeed * 250/1000

			
			if GetDistance(predictDelayPos, myHero) > 100 or GetDistance(myHero,selectedTarget) > GetTrueRangeAutoAttack(myHero) then
			
			if GetDistance(selectedTarget) > GetTrueRangeAutoAttack(myHero) then
				myHero:MoveTo(selectedTarget.x, selectedTarget.z)
			else			
				local point = Vector(selectedTarget) + (Vector(myHero) - Vector(selectedTarget)):normalized() * GetDistance(selectedTarget,selectedTarget.minBBox)
				local rotPoint = RotateAroundPoint(30, selectedTarget, myHero)
				local rotPoint2 = RotateAroundPoint(-30, selectedTarget, myHero)
				
				if GetDistance(rotPoint2, predictDelayPos) > GetDistance(rotPoint, predictDelayPos) then
				myHero:MoveTo(rotPoint.x, rotPoint.z)
				else
				myHero:MoveTo(rotPoint2.x, rotPoint2.z)
				end
				
				--myHero:MoveTo(predictDelayPos.x, predictDelayPos.z)
			end
			
			end
		else--]]
		if GetDistance(selectedTarget) > 75 then
			local point = Vector(selectedTarget) + (Vector(myHero) - Vector(selectedTarget)):normalized() * 75
...
function OnLoad()
	PrintChat("<font color=\"#3F92D2\" >ezCarry v" .. sVersion .. " Loaded - " .. myHero.charName .. "</font>")
	...
	ezSkillShot.GSettings:addParam("PredictionMode", "Prediction Library (Requires Reload)", SCRIPT_PARAM_LIST, 2, {"ezSkillShot", "VPrediction" })
	...
	ezSkillShot[skillShot.spellName]:addParam("PredictPriority", "Prediction Hit Chance", SCRIPT_PARAM_LIST, 2, {"Instant", "Low", "High" })
	AddSpellMenu(ezSkillShot[skillShot.spellName], skillShot)	
	AddHealthPercentMenu(ezSkillShot[skillShot.spellName], skillShot)
